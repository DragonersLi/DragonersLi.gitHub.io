...
var lzxPlayerInit = function () {
    ...
    var webURL = 'https://m.jipaikeji.com/music.php';//
 
        
        $("head").append('<link rel="stylesheet" type="text/css" href="/assets/player/player.css">');
        $("head").append('<link href="/assets/player/font-awesome.css" rel="stylesheet" type="text/css">');
    ...
    $cover.html('<img src="/assets/player/logo.png">');
    $songName.html('<a style="color:#f00">把悲伤里给自己</a>');
    $songFrom.html('<a style="color:#f00">www.dragonersli.gitee.io</a>');
    $songFrom1.html('<a style="color:#f00">DragonersLi音乐播放器</a>');
    ...
    //模式开关
    $('.moshi',$player).click(function(){
        $(this).addClass(cur); 
        if($('.artiststyle .moshi>i').is('.loop')){ 
            $('.artiststyle .moshi>i').removeClass('loop').addClass('random'); //移除顺序，添加随机
            random = true;
            lzxTips.show('随机播放');
            $songFrom2.html('<i class="random fa fa-random current"></i> 随机播放')
            $.cookie("random_play", true)
            console.log('random');
        }else if($('.artiststyle .moshi>i').is('.random')){ 
            $('.artiststyle .moshi>i').removeClass('random').addClass('loop'); //移除随机，添加顺序
            random = false;
            lzxTips.show('顺序播放');
            $songFrom2.html('<i class="loop fa fa-retweet"></i> 顺序播放')
            $.cookie("random_play", false)
    ...
    var lzxLrc = {
        load: function () {
            ...
            setTimeout(function () {
                ...
                $('.switch-ksclrc').show();
                $.ajax({
                    url: webURL + "?act=lyric&songId=" + songSheetList[albumId].songIds[songId] + "&type="
    ...
    function netmusic() {
        audio.src = webURL + "?act=url&songId=" + songSheetList[albumId].songIds[songId]+"&type="+songSheetList[albumId].songTypes[songId];
        $('.switch-down').show();
        ...
        coverImg.error = function () {
            coverImg.src = '/assets/player/logo.png';
            setTimeout(function () {
